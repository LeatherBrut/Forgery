...
using Plane = Forgery.DataStructures.Geometric.Precision.Plane;
using Polygon = Forgery.DataStructures.Geometric.Precision.Polygon;
using Polyhedron = Forgery.DataStructures.Geometric.Precision.Polyhedron;
using PVector3 = Forgery.DataStructures.Geometric.Precision.Vector3;
    ...
    public class VmfBspSourceProvider : IBspSourceProvider
    {
        ...
        public async Task<BspFileLoadResult> Load(Stream stream, IEnvironment environment)
        {
            var task = await Task.Factory.StartNew(async () =>
            {
                ...
                // If the Forgery_native node is found, we should use it to maximise compatibility
                var native = so.FirstOrDefault(x => x.Name == "Forgery_native");
                if (native != null)
                {
        ...
        public Task Save(Stream stream, Map map)
        {
            return Task.Factory.StartNew(() =>
            {
                ...
                var native = new SerialisedObject("Forgery_native");
        ...
        private class VmfSide
        {
            ...
            public VmfSide(SerialisedObject obj)
            {
                ...
                // Older versions of Forgery save vertices, this is entirely optional but why not.
