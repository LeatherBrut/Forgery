...
# Compile the shaders
$content = & "..\Forgery.Rendering\Shaders\compile-shaders.ps1"
$content | Add-Content $log

# Build the project
echo 'Building Solution...'
(& $msbuild '../Forgery.sln' '/p:Configuration=Release' "/p:OutputPath=$outputPath") | Add-Content $log

#(& del '.\Out\Build\*.pdb') | Add-Content $log
(& del '.\Out\Build\*.xml') | Add-Content $log

$version = (Get-Command './Out/Build/Forgery.Editor.exe').FileVersionInfo.ProductVersion
$zipfile = './Out/Forgery.Editor.' + $version + '.zip'
$exefile = './Out/Forgery.Editor.' + $version + '.zip'
$nsifile = './Out/Forgery.Editor.Installer.' + $version + '.nsi'
...
Set-Content $nsifile ((Get-Content '.\Forgery.Editor.Installer.nsi') -replace "\{version\}", $version)

(& $nsis $nsifile) | Add-Content $log

echo 'Creating Version File...'
$date = (Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')
#https://github.com/LogicAndTrick/Forgery/releases/download/pre-alpha/Forgery.Editor.0.1.0.0.zip
$url ='https://github.com/LogicAndTrick/Forgery/releases/download/' + $version + '/Forgery.Editor.' + $version + '.zip'
