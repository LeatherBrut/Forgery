...
namespace Forgery.Providers.Texture.Vtf
{
    ...
    public static class VtfProvider
    {
                    ...
                    for (var face = 0; face < faces; face++)
                    {
                        for (var slice = 0; slice < depth; slice++)
                        {
                            for (var mip = mipNum; mip < mipmapCount; mip++)
                            {
                                ...
                                //img.Save(String.Format(@"D:\Github\Forgery\_Resources\VTF\_test_fr{0}_fa{1}_sl{2}_m{3}.png", frame, face, slice, mip));
                                return img;
                            }
                        }
                    }
        ...
        private static void TransformRgba16161616F(byte[] buffer, BinaryReader br, uint width, uint height)
        {
            ...
            // Fortunately Forgery does not need to care about cubemaps, which is what this format seems to be used for...
            const int a = 6;
            const int r = 0;
            const int g = 2;
            const int b = 4;
